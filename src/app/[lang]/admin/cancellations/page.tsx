            ...
            <tbody>
              {loading ? (
                ...
                <tr key={item.id}>
                  ...
                  <td className="text-sm italic max-w-xs truncate" style={{ color: 'var(--p-text-4)' }} title={item.reason || undefined}>
                    {item.reason ? `"${item.reason}"` : <span className="not-italic" style={{ color: 'var(--p-text-5)' }}>—</span>}
                  </td>
                  <td>
                    <span className="badge badge-red">{text(item.status)}</span>
                  </td>
                </tr>
              ))}
            </tbody>
