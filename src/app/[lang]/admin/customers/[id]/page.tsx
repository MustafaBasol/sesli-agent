              ...
              <tbody className="divide-y divide-gray-800">
                {(!data.reservations || data.reservations.length === 0) ? (
                  ...
                  <tr key={res.id} className="hover:bg-gray-800/20 transition-all">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-[10px] uppercase font-black text-green-500 border border-green-500/20 px-2 py-1 rounded bg-green-500/5">{text(res.status)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {(!data.calls || data.calls.length === 0) ? (
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  <span className="text-[10px] bg-blue-600/10 text-blue-400 border border-blue-500/20 px-2 py-1 rounded font-black uppercase tracking-tighter">{text(call.intent || 'N/A')}</span>
                  <span className="text-[10px] text-gray-500 font-mono">{new Date(call.created_at).toLocaleString(locale)}</span>
                </div>
                <p className="text-sm text-gray-300 leading-relaxed italic font-serif">&quot;{call.summary}&quot;</p>
                <div className="mt-4 text-[10px] text-gray-600 font-black uppercase">{text('Outcome:')} {text(call.outcome || 'N/A')}</div>
              </button>
            ))}
          </div>
        </section>
            ...
            <div className="w-full max-w-4xl max-h-[88vh] overflow-y-auto bg-gray-950 border border-gray-800 rounded-2xl shadow-2xl">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  {[
                    ['Intent', text(selectedCall.intent || '-')],
                    ['Outcome', text(artifacts.endedReason || '-')],
                    ['Phone', artifacts.phone || data.profile.phone_number || '-'],
                    ['Cost', artifacts.cost !== null ? `$${artifacts.cost}` : '-'],
                  ].map(([label, value]) => (
                    <div key={label} className="bg-gray-900 border border-gray-800 rounded-xl p-4">
                      <div className="text-[10px] text-gray-500 uppercase font-black tracking-widest">{text(label)}</div>
