    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
      ...
      <div className="lg:col-span-3">
        <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold text-white">Booking Calendar</h2>
          ...
          <div className="flex gap-3">
            ...
            <div className="flex gap-1 bg-gray-900 p-1 rounded-xl border border-gray-800">
              ...
              <span className="font-bold text-white min-w-[140px] text-center flex items-center justify-center text-sm">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-2xl overflow-hidden">
          ...
          <div className="grid grid-cols-7 gap-1 md:gap-2">
            {padding.map((_, i) => <div key={`p-${i}`} className="h-24 md:h-32 bg-gray-800/10 rounded-2xl" />)}
            {days.map(day => {
              ...
              return (
                <div 
                  ...
                  className={`h-24 md:h-32 p-2 md:p-3 rounded-2xl border cursor-pointer transition-all relative ${
                ...
                >
                  <span className={`text-xs md:text-sm font-black ${isSelected ? 'text-orange-500' : isBlackout ? 'text-red-800' : 'text-gray-600'}`}>{day}</span>
                  <div className="mt-1 flex flex-col gap-1">
                    {isBlackout && <div className="h-1 w-full bg-red-600 rounded-full" />}
                    {dayRes.slice(0, 3).map((r,idx) => (
                      <div key={idx} className="h-1.5 w-1.5 bg-orange-500 rounded-full" />
                    ))}
                    {dayRes.length > 0 && <span className="text-[10px] text-orange-500 font-bold mt-1">{dayRes.length} Bookings</span>}
                  </div>
                </div>
              );
            })}
      ...
      <div className="lg:col-span-1 space-y-6">
        <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 sticky top-8 shadow-2xl h-[calc(100vh-120px)] flex flex-col">
          <header className="mb-6 border-b border-gray-800 pb-4">
            <h3 className="text-xl font-black text-white">{selectedDate || 'Select a day'}</h3>
