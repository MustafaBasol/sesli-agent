 ...
 * Ported from src/lib/vapi-normalizers.ts (Next.js app) so the backend webhook accepts
 * the same field names/formats Vapi has been sending in production.
 *
 ...
 * Use toDigitsOnlyPhone() on the result for the tenant-scoped lookup key.
 */
...
/** Digits-only phone, used as Customer.normalizedPhone — the tenant-scoped lookup key. */
export function toDigitsOnlyPhone(value: string): string {
 ...
 * - Keeps valid ISO dates; corrects a past year to currentYear.
...
/** Build a standard Vapi-compatible missing-fields tool response payload. */
